...
export default function WelcomePage() {
  ...
  const redeem = async (value: string) => {
    ...
    try {
      ...
      sessionStorage.removeItem('risebay_invite_code')
      localStorage.removeItem('risebay_invite_code')
      toast.success('You are in. Welcome to RiseBay.')
      navigate('/browse')
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const saved =
      sessionStorage.getItem('risebay_invite_code') ??
      localStorage.getItem('risebay_invite_code')
      ...
      <Container className="max-w-md">
        <Card>
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-slate-400">
            This account cannot use RiseBay right now. If you think that is a mistake, reply to
    ...
    <Container className="max-w-md">
      <Card>
        ...
        <p className="mt-1 text-sm text-gray-600 dark:text-slate-400">
          RiseBay is invite only. Enter the code a member sent you.
        </p>
